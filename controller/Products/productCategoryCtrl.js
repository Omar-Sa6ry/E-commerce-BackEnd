...
const PCategory = require('../../models/Product/productCategoryModel')
...
const createCategory = asyncHandler(async (req, res, next) => {
  const getBrnd = await PCategory.findOne({ title: req.body.title })
  if (!getBrnd) {
...
const getAllCategories = asyncHandler(async (req, res, next) => {
  ...
  let query = PCategory.find(JSON.parse(queryStr))
  ...
  if (page) {
    const categoryCount = await PCategory.countDocuments()
